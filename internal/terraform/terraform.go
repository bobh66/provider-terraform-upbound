...
func (h Harness) GenerateChecksum(ctx context.Context) (string, error) {
	command := "/usr/bin/find . -path ./.terraform/terraform.tfstate -prune -o -path ./.git -prune -o -path ./.terraform/providers -prune -o -type f -exec /usr/bin/md5sum {} + | LC_ALL=C /usr/bin/sort | /usr/bin/md5sum | /usr/bin/awk '{print $1}'"
